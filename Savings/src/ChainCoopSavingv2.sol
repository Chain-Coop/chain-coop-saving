...
import {IChainCoopSaving} from "./interface/IchainCoopSavingv2.sol";
import {LibChainCoopSaving} from "./lib/LibChainCoopSavingv2.sol";
...
error PoolAlreadyOnAave(bytes32 _poolId);
error PoolNotOnAave(bytes32 _poolId);
    ...
    IChainCoopSaving
{
    ...
    /**
     * @notice Allow Opening a saving pool with initial contribution (without Aave integration)
     */
    function openSavingPool(
        ...
        SavingPool memory pool = SavingPool({
            ...
            aaveDepositAmount: 0, // No Aave deposit initially
            locktype: _locktype,
            isGoalAccomplished: accomplished,
            isStoped: false,
            aaveEnabled: false // Disabled by default
        });
     ...
     * @param _poolId The pool to enable Aave for
     */
    function enableAaveForPool(bytes32 _poolId) external nonReentrant {
        address sender = _msgSender();
        ...
        if (pool.aaveEnabled) {
            revert PoolAlreadyOnAave(_poolId);
        }

        if (!aaveConfigs[pool.tokenToSaveWith].isConfigured) {
            revert AaveNotConfigured();
        }

        AaveConfig memory config = aaveConfigs[pool.tokenToSaveWith];

        // Deposit current pool balance to Aave
        config.pool.supply(
        ...
        // Update pool state
        pool.aaveEnabled = true;
        pool.aaveDepositAmount = pool.amountSaved;
        totalAaveDeposits[pool.tokenToSaveWith] += pool.amountSaved;

        emit AaveDepositEnabled(sender, _poolId, pool.amountSaved);
    }
    ...
    function withdraw(bytes32 _poolId) external nonReentrant {
        ...
        if (pool.aaveEnabled && pool.aaveDepositAmount > 0) {
            ...
            uint256 currentATokenBalance = getPoolAaveBalance(_poolId);
     ...
     */
    function claimAaveRewards(bytes32 _poolId) external onlyOwner {
        address sender = _msgSender();
        SavingPool storage pool = poolSavingPool[_poolId];
    ...
    function getPoolAaveBalance(
        ...
        uint256 totalATokenBalance = config.aToken.balanceOf(address(this));
    ...
    function getUserContributions(
        ...
        for (uint256 i = 0; i < userPoolCount; i++) {
            ...
            if (pool.aaveEnabled) {
                aaveBalance = getPoolAaveBalance(poolId);
            }
    ...
    function getPoolYield(bytes32 _poolId) external view returns (uint256) {
        ...
        uint256 currentBalance = getPoolAaveBalance(_poolId);
        return
